...
public class MansionPopulator extends SingleMegaChunkStructurePopulator {
    ...
    public boolean canSpawn(@NotNull TerraformWorld tw, int chunkX, int chunkZ, BiomeBank biome) {
        ...
        if(Math.pow(chunkX*16,2) + Math.pow(chunkZ*16,2) < Math.pow(config.STRUCTURES_MANSION_MINDISTANCE,2))
            return false;
    ...
    public int getChunkBufferDistance() {
    	return config.STRUCTURES_MANSION_CHUNK_EXCLUSION_ZONE;
    }

    @Override
    public boolean isEnabled() {
        return BiomeBank.isBiomeEnabled(BiomeBank.DARK_FOREST)
			   && config.STRUCTURES_MANSION_ENABLED;
    }
}
