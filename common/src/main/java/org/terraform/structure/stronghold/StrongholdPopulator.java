...
public class StrongholdPopulator extends SingleMegaChunkStructurePopulator {
            ...
            for (int z = data.getChunkZ() * 16; z < data.getChunkZ() * 16 + 16; z++) {
                for (int[] pos : positions) {
                    if (areCoordsEqual(pos, x, z)) {
                        ...
                        int y = GenUtils.randInt(config.STRUCTURES_STRONGHOLD_MIN_Y, config.STRUCTURES_STRONGHOLD_MAX_Y);
                        
                        //Attempt to force strongholds further underground if
                        //they're above the surface.
                        if(y + 18 > GenUtils.getHighestGround(data, x, z)) {
                        	if(y > config.STRUCTURES_STRONGHOLD_FAILSAFE_Y) y = config.STRUCTURES_STRONGHOLD_FAILSAFE_Y;
                        }
    ...
    public boolean isEnabled() {
        return config.STRUCTURES_STRONGHOLD_ENABLED;
    }
