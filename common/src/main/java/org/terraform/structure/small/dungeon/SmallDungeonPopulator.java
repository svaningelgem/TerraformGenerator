...
public class SmallDungeonPopulator extends MultiMegaChunkStructurePopulator {
    ...
    public void populate(@NotNull TerraformWorld tw, @NotNull PopulatorDataAbstract data) {
        ...
        if (totalHeight / 256 <= config.STRUCTURES_DROWNEDDUNGEON_MIN_DEPTH
                && GenUtils.chance(tw.getHashedRand(1223, data.getChunkX(), data.getChunkZ()), config.STRUCTURES_DROWNEDDUNGEON_CHANCE, 1000)) {
            //Only spawn these in full oceans
            if (!config.STRUCTURES_DROWNEDDUNGEON_ENABLED)
                return;
            new DrownedDungeonPopulator().populate(tw, data);
        } else {
            if (!config.STRUCTURES_UNDERGROUNDDUNGEON_ENABLED)
                return;
    ...
    public int[][] getCoordsFromMegaChunk(@NotNull TerraformWorld tw, @NotNull MegaChunk mc) {
    	int num = config.STRUCTURES_DUNGEONS_COUNT_PER_MEGACHUNK;
    ...
    public boolean isEnabled() {
        return config.STRUCTURES_DROWNEDDUNGEON_ENABLED || config.STRUCTURES_UNDERGROUNDDUNGEON_ENABLED;
    }
}
