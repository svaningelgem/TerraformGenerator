...
public class DesertWellPopulator extends MultiMegaChunkStructurePopulator {
    ...
    private boolean rollSpawnRatio(@NotNull TerraformWorld tw, int chunkX, int chunkZ) {
        return GenUtils.chance(tw.getHashedRand(chunkX, chunkZ, 8291374),
                (int) (TConfig.STRUCTURES_DESERTWELL_SPAWNRATIO
                        .getDouble() * 10000),
                10000);
    }
    @Override
    ...
    public int[][] getCoordsFromMegaChunk(@NotNull TerraformWorld tw, @NotNull MegaChunk mc) {
        int num = config.STRUCTURES_DESERTWELL_COUNT_PER_MEGACHUNK;
    ...
    public boolean isEnabled() {
        return config.STRUCTURES_DESERTWELL_ENABLED
			   && (config.BIOME_DESERT_WEIGHT > 0 ||
				   config.BIOME_BADLANDS_WEIGHT > 0);
    }
