        ...
        for(Holder<Structure> holder:holderset) {
            ...
            else if(!config.getBoolean(TConfig.DEVSTUFF_VANILLA_LOCATE_DISABLE))
            {
                if (holder.a().getClass() == OceanMonumentStructure.class) { //Monument
                    ...
                    int[] coords = StructureLocator.locateSingleMegaChunkStructure(tw, pX, pZ, new TrialChamberPopulator(), config.getInt(TConfig.DEVSTUFF_VANILLA_LOCATE_TIMEOUTMILLIS));
